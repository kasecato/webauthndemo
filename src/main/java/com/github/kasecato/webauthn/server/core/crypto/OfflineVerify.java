...
package com.github.kasecato.webauthn.server.core.crypto;
...
public class OfflineVerify {
    ...
    public static AttestationStatement parseAndVerify(final String signedAttestationStatement) {
        ...
        final AttestationStatement stmt = (AttestationStatement) jws.getPayload();
        return stmt;
    }
     ...
     * @return
     */
    private static boolean verifyHostname(final String hostname, final X509Certificate leafCert) {
        try {
    ...
    private static void process(final String signedAttestationStatement) {
        final AttestationStatement stmt = parseAndVerify(signedAttestationStatement);
        if (stmt == null) {
