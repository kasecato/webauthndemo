...
public abstract class AttestationStatement {
     ...
     */
    public static AttestationStatement decode(
            final String fmt,
            final DataItem attStmt) {

        if (fmt.equals("fido-u2f")) {
            final FidoU2fAttestationStatement stmt = FidoU2fAttestationStatement.decode(attStmt);
            return stmt;
        } else if (fmt.equals("android-safetynet")) {
            final AndroidSafetyNetAttestationStatement stmt;
            try {
