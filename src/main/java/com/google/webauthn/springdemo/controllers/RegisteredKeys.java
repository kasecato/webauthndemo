...
public class RegisteredKeys {
    ...
    @Autowired
    public RegisteredKeys(UserNService userNService) {
    ...
    protected Set<RegisteredKey> getRegisteredKeys(final Authentication authentication)
        ...
        final Set<CredentialStoreN> savedCreds = user.getCredentials();

        final Set<RegisteredKey> registeredKeys = new HashSet<>();
        for (final CredentialStoreN c : savedCreds) {
            final AuthenticatorAttestationResponseN response = new AuthenticatorAttestationResponseN(c.getAttestationObjectBytes());
