...
public abstract class CredentialPublicKey {
     ...
     */
    public static CredentialPublicKey decode(final byte[] cbor) throws CborException {
        final List<DataItem> dataItems = CborDecoder.decode(cbor);
        if (dataItems.size() < 1 || !(dataItems.get(0) instanceof Map)) {
            return null;
        }

        final Map map = (Map) dataItems.get(0);
        // If there are 4 keys in the map, the key should be RSA. If there are 5, then it is ECC.
        if (map.getKeys().size() == 4) {
            RsaKey rsaKey = new RsaKey();

            for (final DataItem d : map.getKeys()) {
                int tmp = 0;
            ...
            final EccKey eccKey = new EccKey();
